...
exports.WebglPtFieldObj = exports.WebglPtFieldKind = exports.WebglPtFieldShader = void 0;
...
`;
class WebglPtFieldShader {
    constructor() {
        this.id = "weblgl_pt_field";
        this.extra_uniforms = ["data"];
...
exports.WebglPtFieldShader = WebglPtFieldShader;
var WebglPtFieldKind;
(function (WebglPtFieldKind) {
    WebglPtFieldKind[WebglPtFieldKind["Uniform"] = 0] = "Uniform";
    WebglPtFieldKind[WebglPtFieldKind["SphereMap"] = 1] = "SphereMap";
    WebglPtFieldKind[WebglPtFieldKind["CubeSurface"] = 2] = "CubeSurface";
    WebglPtFieldKind[WebglPtFieldKind["CubeSurfaceSphereMap"] = 3] = "CubeSurfaceSphereMap";
})(WebglPtFieldKind || (exports.WebglPtFieldKind = WebglPtFieldKind = {}));
class WebglPtFieldObj {
    /** Construct with a regular grid; set num_points to equal nx*ny*nz */
    constructor(num_points, nx = 1024, ny = 1024, nz = 1) {
        this.position_buf = null;
        this.indices_buf = null;
        this.num_points = 0;
        this.nx = 1024; // 16 bits
        this.ny = 1024; // 16 bits
        this.nz = 1; // 12 bits
        this.min_sz = 0; // 2 bits
        this.max_sz = 0; // 2 bits
        this.field_kind = WebglPtFieldKind.Uniform;
        this.pt_random_weight = 0; // 8 bits
        this.sz_random_weight = 0; // 4 bits
        this.random_seed = 0x1234678;
        this.encoded = false;
    ...
    static sphere_surface(num_points, sparsity = 1, nx = 0, ny = 0) {
        if (nx < 1) {
    ...
    set_size_range(min_sz, max_sz) {
        this.min_sz = min_sz;
        this.max_sz = max_sz;
        this.encoded = false;
        return this;
    }
    set_sz_weight(sz_random_weight) {
    ...
    webgl_create(webgl) {
        ...
        webgl.bufferData(webgl.ARRAY_BUFFER, new Float32Array([0, 0, 0, this.random_seed]), webgl.STATIC_DRAW);
        this.indices_buf = webgl.createBuffer();
        webgl.bindBuffer(webgl.ELEMENT_ARRAY_BUFFER, this.indices_buf);
        webgl.bufferData(webgl.ELEMENT_ARRAY_BUFFER, new Uint16Array([0]), webgl.STATIC_DRAW);
    }
    webgl_set_uniforms(webgl) {
        if (!this.encoded) {
            this.do_encode();
        }
        webgl.set_uniform_ivec4(web_gl_js_1.WebglUniform.Extra0, this.data);
    }
    webgl_draw(webgl) {
        webgl.bindBuffer(webgl.ARRAY_BUFFER, this.position_buf);
        ...
        webgl.bindBuffer(webgl.ELEMENT_ARRAY_BUFFER, this.indices_buf);
        webgl.drawElementsInstanced(webgl.POINTS, 1, webgl.UNSIGNED_SHORT, 0, this.num_points);
    }
}
exports.WebglPtFieldObj = WebglPtFieldObj;
//# sourceMappingURL=web_gl_pt_field.js.map