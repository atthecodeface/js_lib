...
export class Quaternion {
  ...
  length_sq(): number {
    return dot(this.rijk, this.rijk);
  }
  ...
  set_scale_add_scaled(scale: number, other: this, oscale: number): void {
    set_scale_add_scaled(this.rijk, scale, other.rijk, oscale);
  }
  ...
  set_of_axis_angle(axis: [number, number, number], angle: number) {
    const l = Math.sqrt(dot(axis, axis));
    if (l < 1e-6) {
      this.rijk = [1, 0, 0, 0];
    } else {
